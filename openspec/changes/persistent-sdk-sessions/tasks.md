...
- [ ] 5.2 Branch the two non-streaming `query()` call sites in `server.ts` (lines ~831 / ~870) to route through `SessionRuntime` when the flag is on and the request is not an undo/fork
- [ ] 5.3 Branch the two streaming `query()` call sites in `server.ts` (lines ~1252 / ~1288) to route through `SessionRuntime` when the flag is on and the request is not an undo/fork
...
- [ ] 5.5a Options classifier (pure): takes request options, returns `{ reopenCriticalHash, inPlaceUpdates }` where `inPlaceUpdates` is a list of `{ kind: "setModel" | "applyFlagSettings", payload }`. Unit-testable without SDK.
...
- [ ] 5.6 Add cold-reattach logic: if `sessionStore` has the session but the live-query map doesn't, start `query({ resume: claudeSessionId })`. Must preserve the in-flight request's user message and its SSE response stream so the client sees one continuous response even when the runtime was swapped.
- [ ] 5.7 Integration test with mocked `Query` covering: first-turn new, second-turn warm, concurrent turns serialized, reopen on options drift, undo close+reopen, cold-reattach after simulated crash
...
- [ ] 5.12 Implement the deferred MCP handler pattern for passthrough (design §D11 rewritten after task §1d; replaces the old drain-in-background plan):
  - [ ] 5.12a Add `pendingExecutions: Map<string, { resolve: (content: string) => void; reject: (err: unknown) => void; createdAt: number }>` to `SessionRuntime`
  - [ ] 5.12b Rewrite `createPassthroughMcpServer` so handlers are closures over the runtime's `pendingExecutions`: on call, create a pending entry keyed by `tool_use_id` and return a `Promise<MCPToolResult>` that resolves to `{ content: [{ type: "text", text: <resolved-content> }] }`
...
- [ ] 5.13 Cache-control sanitizer invariant (surfaced by task §1c live spike, captured in design §D10): every user content object pushed into a `SessionRuntime.inputQueue` MUST pass through `stripCacheControlDeep` (or equivalent) first, or persistent conversations exceed Anthropic's 4-cache_control-block limit after 4 turns. Unit test the invariant at the server-wiring layer
