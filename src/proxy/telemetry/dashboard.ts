...
function render(s, reqs, logs) {
  ...
  for (const r of reqs) {
    ...
    var lineageBadge = '';
    if (r.lineageType) {
      var lColors = {continuation:'var(--green)',compaction:'var(--yellow)',undo:'var(--purple)',diverged:'var(--red)','new':'var(--muted)'};
      var lc = lColors[r.lineageType] || 'var(--muted)';
      lineageBadge = '<span style="color:' + lc + '">' + r.lineageType + '</span>';
      if (r.sdkSessionId) lineageBadge += ' <span class="mono" style="color:var(--muted)">' + r.sdkSessionId.slice(0,8) + '</span>';
      if (r.messageCount != null) lineageBadge += ' <span style="color:var(--muted)">(' + r.messageCount + ')</span>';
    }
