...
/** Shape of `mcpServers` passed into the SDK query options in these tests */
type CapturedMcpServersEntry = { opencode: { _id: number } };
let capturedMcpServers: CapturedMcpServersEntry[] = [];

mock.module("@anthropic-ai/claude-agent-sdk", () => ({
  query: (params: { options?: { mcpServers?: CapturedMcpServersEntry } }) => {
...
describe("MCP server per-request lifecycle", () => {
  ...
  it("should pass unique MCP server instances to each query call", async () => {
    ...
    if (!server1 || !server2) {
      throw new Error("expected two MCP server instances");
    }
