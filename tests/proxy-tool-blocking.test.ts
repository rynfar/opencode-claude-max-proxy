...
import { assistantMessage, type TestFetchApp as FetchApp } from "./helpers";
...
const ALL_BLOCKED = [
  ...new Set([...BLOCKED_BUILTIN_TOOLS, ...CLAUDE_CODE_ONLY_TOOLS]),
];
...
function getDisallowedToolsList(
  params: Record<string, unknown> | null,
): string[] {
  if (!params) return [];
  const raw = params.options;
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return [];
  const disallowed = (raw as Record<string, unknown>).disallowedTools;
  if (!Array.isArray(disallowed)) return [];
  return disallowed.filter((t): t is string => typeof t === "string");
}

async function sendRequest(app: FetchApp, stream: boolean) {
...
function assertAllToolsBlocked(
  params: Record<string, unknown> | null,
  _label: string,
) {
  const disallowed = getDisallowedToolsList(params);
...
describe("Tool blocking: consistency across all paths", () => {
  it("all 4 paths produce identical disallowedTools lists", async () => {
    ...
    results.push({
      mode: "normal/non-stream",
      tools: [...getDisallowedToolsList(params)].sort(),
    });
    ...
    if (!baseline) {
      throw new Error("expected baseline disallowed tools");
    }
